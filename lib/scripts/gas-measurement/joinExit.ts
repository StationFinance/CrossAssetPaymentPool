...
import { printGas, setupEnvironment, getWeightedPool, getStablePool, getStationPool } from './misc';
...
async function main() {
  ...
  for (let numTokens = 2; numTokens <= 8; numTokens += 2) {
    printTokens('Station pool', numTokens);
    await joinAndExitPool(() => getStationPool(vault, tokens, numTokens), numTokens, true);
  }
  console.log('\n');
  ...
  for (let numTokens = 2; numTokens <= 8; numTokens += 2) {
    printTokens('Station pool', numTokens);
    await joinAndExitPool(() => getStationPool(vault, tokens, numTokens), numTokens, false);
  }
  console.log('\n');
  ...
  for (let numTokens = 2; numTokens <= 8; numTokens += 2) {
    printTokens('Station pool', numTokens);
    await joinAndExitPool(() => getStationPool(vault, tokens, numTokens), numTokens, true, numberJoinsExits);
  }
  console.log('\n');
  ...
  for (let numTokens = 2; numTokens <= 8; numTokens += 2) {
    printTokens('Station pool', numTokens);
    await joinAndExitPool(() => getStationPool(vault, tokens, numTokens), numTokens, false, numberJoinsExits);
  }
  console.log('\n');
...
async function joinAndExitPool(
  ...
  const pool: Contract = await ethers.getContractAt('StationPool', poolAddress);
  let receipt;
  let bpt;
