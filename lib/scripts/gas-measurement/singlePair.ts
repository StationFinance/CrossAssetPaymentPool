...
async function main() {
  ...
  console.log(`\n# Station Pools with 2 tokens`);

  await singlePair(() => getStationPool(vault, tokens, 2), false);
  await singlePair(() => getStationPool(vault, tokens, 2), true);

  console.log(`\n# Station Pools with 4 tokens`);

  await singlePair(() => getStationPool(vault, tokens, 4), false);
  await singlePair(() => getStationPool(vault, tokens, 4), true);
}
