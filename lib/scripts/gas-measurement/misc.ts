...
import { encodeJoinStationPool } from '../../helpers/stationPoolEncoding';
...
export async function deployPool(vault: Contract, tokens: TokenList, poolName: PoolName): Promise<string> {
  ...
  } else if (poolName == 'StationPool') {
    const amp = bn(10);

    pool = await deployPoolFromFactory(vault, admin, 'StationPool', {
      from: creator,
      parameters: [tokenAddresses, amp],
    });

    joinUserData = encodeJoinStationPool({ kind: 'Init', amountsIn: tokenAddresses.map(() => initialPoolBalance) });
  } else {
...
export async function getStationPool(
  vault: Contract,
  tokens: TokenList,
  size: number,
  offset?: number
): Promise<string> {
  return deployPool(vault, pickTokens(tokens, size, offset), 'StationPool');
}
