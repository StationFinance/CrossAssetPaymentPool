...
import { getWeightedPool, getStablePool, getStationPool, printGas, setupEnvironment, tokenSymbols } from './misc';
...
async function main() {
  ...
  console.log(`\n# Station Pool with 2 tokens`);

  await multihop((index: number) => getStationPool(vault, tokens, 2, index), false);
  await multihop((index: number) => getStationPool(vault, tokens, 2, index), true);

  console.log(`\n# Station Pool with 4 tokens`);

  await multihop((index: number) => getStationPool(vault, tokens, 4, index), false);
  await multihop((index: number) => getStationPool(vault, tokens, 4, index), true);
}
